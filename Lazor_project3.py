...
def read_file(filename):
    ...
    **Returns**
        ...
        grid_str: *list of str*
            a list of strings describing the grid to be
            constructed. Each character is a cell in the board.
        num_blocks: *list of ints*
            list containing the number of reflective, opaque and
            refractive blocks (in that order) that are available
            to use to solve the board
...
def solve_game(filename):
    '''
    Function that solves the game. This function takes a
    game as an input (this game object already has all the
    blocks available placed in a specific arrangement) and
    turns on all the lasers (shoot()). It will then calculate/obtain
    all the path trajectories from each laser and compare the points
    in the trajectories to the points that we are targetting. If all
    the target points are included in the trajectories then the game is
    solved and the function returns an image representation (or text to
    simplify) showing which block arrangement solves the puzzle. If any of
    the target points is missing in the trajectories then the puzzle is not
    solved, the function will regenerate the game() object and check to see
    if this new arrangement solves the board.
    ...
    **Parameters**
    ...
    # Reads and extracts data from file
    board_str, num_blocks, lasers_pos, lasers_dir,\
    ...
    grid_List = []
    while solved is False and iterations <= MAX_ITERATIONS:
        ...
        # if the block at rand_x, rand_y is available put
        # block there. Else get another random number for x and y
        # if the block is adjacent to a target it becomes
        # invalid for reflect and opaque blocks
        ...
        for i in targets:
            ...
            # where m and n are the corresponding coordinates in the
            # grid and board respectively
            converted_to_blocks = [[int(
                (grid_points[0][0] - 1) / 2),
                int((grid_points[0][1] - 1) / 2)],
                [int(
                    (grid_points[1][0] - 1) / 2), int(
                    (grid_points[1][1] - 1) / 2)]]
            ...
            for i in range(len(game1.available_blocks)):
                ...
                while is_placed is False:
                    ...
                    if game1.board[rand_y][rand_x] is not VALID:
                        ...
                        print('elif')
                        if (
                            next_block.block_type == OPAQUE or
                                next_block.block_type == REFLECT):

                            print('if OPAQUE or REFLECT')
                            if any(elem in all_invalid_adj_blocks
                                    for elem in next_rand) is True:

                                print('if any position is invalid adj')
                                rand_x = random.randint(
                                    0, len(game1.board[0]) - 1)
        ...
        # while loop: while there are lasers that
        # have not been shot do a for loop to shoot all lasers
        ...
        if game1.hit_all_targets() is True:
            solved = True
            print(iterations)
            ...
            print("Loser")
            print("The board couldn't be solved" and
                " under the max specified iterations")
            print("Try running it again or increase "
                " the max allowed iterations per run")
            solved = False
...
class Game():
    ...
    def __init__(self, board_list_of_str,
        num_blocks, lasers_pos, lasers_dir, targets):
        '''
        ...
        # Create blocks and lasers available for the
        # game, based on file info
        ...
        for i in range(len(self.lasers_pos)):
            laser = Laser(
                self.lasers_pos[i][0], self.lasers_pos[i][1],
                self.lasers_dir[i][0],
                self.lasers_dir[i][1])
            self.available_lasers.append(laser)

    def create_board(self):
        # read in file and change the existing board list
        # to match the available positions in the file.
        board = [[
            VALID for _ in range(self.b_width)] for _ in range(self.b_height)]
    ...
    def create_grid(self):
        ...
        '''
        grid = [
            [VALID for _ in range(self.g_width)] for _ in range(self.g_height)]
        ...
        for w in range(self.b_width):
            for h in range(self.b_height):
                if self.board[h][w] == VALID:
                    ...
                    uselessgrid, grid_faces =\
                        self.cardinal(
                            grid, grid_faces, 2 * w + 1, 2 * h + 1, VALID)
                    continue
                if self.board[h][w] == INVALID:
                    grid[2 * h + 1][2 * w + 1] = INVALID
                    grid, grid_faces =\
                        self.cardinal(
                            grid, grid_faces, 2 * w + 1, 2 * h + 1, INVALID)
                if self.board[h][w] == REFLECT:
                    # or self.board[h][w].block_type == REFLECT:
                    grid[2 * h + 1][2 * w + 1] = REFLECT
                    grid, grid_faces =\
                        self.cardinal(
                            grid, grid_faces, 2 * w + 1, 2 * h + 1, REFLECT)
                if self.board[h][w] == OPAQUE:
                    # or self.board[h][w].block_type == OPAQUE:
                    grid[2 * h + 1][2 * w + 1] = OPAQUE
                    grid, grid_faces =\
                        self.cardinal(
                            grid, grid_faces, 2 * w + 1, 2 * h + 1, OPAQUE)
                if self.board[h][w] == REFRACT:
                    # or self.board[h][w].block_type == REFRACT:
                    grid[2 * h + 1][2 * w + 1] = REFRACT
                    grid, grid_faces =\
                        self.cardinal(
                            grid, grid_faces, 2 * w + 1, 2 * h + 1, REFRACT)
    ...
    def cardinal(self, grid, grid_faces, x, y, value):
        ...
        **Parameters**
            ...
            value: *int or codeword (see above)*
                value of the board block that we want to give to the
                grid positions. Can be valid (0), invalid (1),
                reflective block (4), opaque block (3),
                refractive block (2)
    ...
    def within_bounds(self, matrix, x, y):
        ...
        If it is in the board take the next step and see if it bumps
        into anything.
    ...
    def shoot(self, laser):
        ...
        if self.grid[current_y][current_x] == REFLECT:
            if (self.grid_faces[current_y][
                current_x] == 2 and self.grid[current_y][
                current_x + xdir] == REFLECT) or (self.grid_faces[current_y][
                    current_x] == 1 and self.grid[current_y + ydir][
                    current_x] == REFLECT):
        ...
        if self.grid[current_y][current_x] == OPAQUE:
            if (self.grid_faces[current_y][
                current_x] == 2 and self.grid[current_y][
                current_x + xdir] == OPAQUE) or (self.grid_faces[current_y][
                    current_x] == 1 and self.grid[current_y + ydir][
                    current_x] == OPAQUE):
                face_number = self.get_block_face(current_x, current_y)
                laser.absorb(xdir, ydir, face_number)
        if self.grid[current_y][current_x] == REFRACT:
            if (self.grid_faces[current_y][current_x] == 2 and self.grid[
                current_y][current_x + xdir] == REFRACT) or (self.grid_faces[
                    current_y][current_x] == 1 and self.grid[current_y + ydir][
                    current_x] == REFRACT):
                face_number = self.get_block_face(current_x, current_y)
        ...
        if laser.get_trajectory()[-1] != [-2, -2]:
            ...
            while self.within_bounds(
                self.grid, next_x, next_y) and laser.get_trajectory()[
                    -1] != [-2, -2]:
                print(xdir)
                print(ydir)
                ...
                if self.grid[current_y][current_x] == REFLECT:
                    ...
                    try:
                        if (self.grid_faces[current_y][
                            current_x] == 2 and self.grid[current_y][
                            current_x + xdir] == REFLECT) or (self.grid_faces[
                                current_y][current_x] == 1 and self.grid[
                                    current_y + ydir][current_x] == REFLECT):
                            face_number = self.get_block_face(
                                current_x, current_y)
                            xdir, ydir = laser.reflect(xdir, ydir, face_number)
                ...
                if self.grid[current_y][current_x] == OPAQUE:
                    if (self.grid_faces[current_y][
                        current_x] == 2 and self.grid[current_y][
                        current_x + xdir] == OPAQUE) or (
                        self.grid_faces[current_y][
                            current_x] == 1 and self.grid[current_y + ydir][
                                current_x] == OPAQUE):
                        face_number = self.get_block_face(current_x, current_y)
                        laser.absorb(xdir, ydir, face_number)
...
class Laser():
    ...
    def get_trajectory(self):
        ...
        **Returns**
            path: *list of tuples*
                list with every position tuple
                (x,y) that the beam has gone through.
        '''
        return self.path
    ...
    def absorb(self, xdir, ydir, face_number):
        '''
        Function the describes what happens when a laser beam
        hits an opaque block: light is absorbed.
        ...
        **Returns**
            new_xdir: *int*
                Outbound laser direction. Set to zero because the
                laser doesnt have any direction past this point
            new_ydir: *int*
                outbound laser direction. Set to zero because the
                laser doesnt have any direction past this point
            x: *int*
...
if __name__ == "__main__":
    ...
    # solved = solve_game("yarn_5.bff")
    # solved = solve_game("showstopper_4.bff")
    print('in main')
    print(solved)
